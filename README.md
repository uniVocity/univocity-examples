...
Any data entity accessible through a data store can be read using the exact same interface: [Entity](http://github.com/uniVocity/univocity-api/tree/master/src/main/java/com/univocity/api/entity/Entity.java). It doesn't matter if the entity is a file, a database table or any other [CustomDataEntity](http://github.com/uniVocity/univocity-api/tree/master/src/main/java/com/univocity/api/entity/custom/CustomDataEntity.java) you define. Once the `beginReading` method is invoked, a concurrent process will start to read data from your entity into an in-memory buffer. All you need to do is to consume the rows, as demonstrated in the following example:
...
Reading data from `Object` arrays can be annoying, but the [Entity](http://github.com/uniVocity/univocity-api/tree/master/src/main/java/com/univocity/api/entity/Entity.java) offers a few useful methods to make the reading process less cumbersome:
